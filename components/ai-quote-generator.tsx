...
interface AIQuoteGeneratorProps {
  emailContent: string;
  requestId: string; // Added to identify the QuotationRequest
}

export function AIQuoteGenerator({ emailContent, requestId }: AIQuoteGeneratorProps) {
  ...
  const startAIProcessing = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/api/ai-processing/generate-quote', { requestId });
  ...
  const handleCreateQuotation = async () => {
    ...
    try {
      // The quotation is already created in the backend (in generateQuote).
    ...
    <div className="space-y-6">
      <Card className="border-0 shadow-sm">
        ...
        <CardContent className="space-y-4">
          ...
          <Button onClick={startAIProcessing} disabled={isProcessing || !requestId} className="bg-blue-600 hover:bg-blue-700">
            <Zap className="h-4 w-4 mr-2" />
            {isProcessing ? 'Processing...' : 'Generate Quote with AI'}
          </Button>
      ...
      {isProcessing && (
        <Card className="border-0 shadow-sm">
          ...
          <CardContent>
            ...
            <p className="text-sm text-gray-600 mt-2">Generating quotation, please wait...</p>
          </CardContent>
        </Card>
      )}

      {generatedQuote && (
        <Card className="border-0 shadow-sm border-green-200">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-green-800">
              <CheckCircle className="h-5 w-5" />
              AI-Generated Quotation
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex gap-3 pt-4 border-t">
              <Button className="bg-green-600 hover:bg-green-700" onClick={handleCreateQuotation}>
                <FileText className="h-4 w-4 mr-2" />
                Create Quotation
              </Button>
              <Button variant="outline">Review & Edit</Button>

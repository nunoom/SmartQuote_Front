...
import { useAuth } from '@/lib/auth/auth-context';
import { useLanguage } from '@/lib/i18n/language-context';
import { Check, X, Eye, MessageSquare, Clock, User, Loader2, CheckCircle } from 'lucide-react';
import toast from 'react-hot-toast';
...
export function ApprovalsList() {
  ...
  const fetchApprovals = useCallback(async () => {
    ...
    try {
      ...
      // Map API response to Approval interface (only PENDING requests)
      const sanitizedApprovals: Approval[] = response.data
      ...
      console.error('Error fetching approvals:', err);
      const errorMessage = err.response?.data?.message || t('failed_to_load_approvals');
  ...
  const handleApprove = async (approvalId: string) => {
    ...
    try {
      ...
      if (response.data.quotationId) {
        // Navigate to generated quotation if provided
        window.location.href = `/quotations/${response.data.quotationId}`;
      }
    } catch (err: any) {
      console.error('Error approving:', err);
      const errorMessage = err.response?.data?.message || t('failed_to_approve');
      toast.error(errorMessage);
    }
  };
  ...
  const handleReject = async (approvalId: string) => {
    ...
    try {
      ...
      console.error('Error rejecting:', err);
      const errorMessage = err.response?.data?.message || t('failed_to_reject');
      toast.error(errorMessage);
    }
  };
            ...
            <CardContent className="p-6">
              <div className="space-y-4">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-lg font-semibold text-gray-200">{request.description}</h3>
                      <Badge
                        className={
                          approval.status === 'pending'
                            ? 'bg-yellow-900/20 text-yellow-400 border-yellow-900/30'
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="flex items-center gap-2">
                        <User className="h-4 w-4 text-yellow-400/70 hover:rotate-6 transition-transform duration-200" />
                        <div>
                          <p className="font-medium text-gray-200">{request.requester}</p>
                          <p className="text-sm text-yellow-400/70">{request.email}</p>
                        </div>
                      </div>

                      <div className="flex items-center gap-2">
                        <Clock className="h-4 w-4 text-yellow-400/70 hover:rotate-6 transition-transform duration-200" />
                        <div>
                          <p className="text-sm text-yellow-400/70">{t('requested')}</p>
                      ...
                      <div>
                        ...
                        <p className="font-medium text-yellow-400">{t('pending_approval')}</p>
                      </div>
                    </div>
                  </div>
                ...
                {isExpanded && (
                  <div className="border-t border-yellow-900/30 pt-4 space-y-4">
                    <div>
                      <h4 className="font-medium text-gray-200 mb-2">{t('request_details')}</h4>
                      <div className="bg-neutral-900 p-3 rounded-md border border-yellow-900/30">
                        <p className="text-sm text-yellow-400/70 whitespace-pre-wrap">{request.description}</p>
                      </div>
                    </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {request.attachments.map((attachment) => (
                            ...
                            >
                              <CheckCircle className="h-4 w-4 mr-2 hover:rotate-6 transition-transform duration-200" />
                              {attachment.fileName}
                            </a>
                          ))}
                        </div>
                ...
                {approval.status !== 'pending' && (
                  <div className="border-t border-yellow-900/30 pt-4">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-yellow-400/70">
                        {t(approval.status)} {t('by')} {approval.approvedBy || 'Unknown'}
                        {approval.approvedAt && ` ${t('on')} ${new Date(approval.approvedAt).toLocaleDateString('pt-AO')}`}
                      </span>
                    </div>
                    {approval.comments && (
                      <div className="mt-2 p-3 bg-neutral-900 rounded-md border border-yellow-900/30">
